...
public class Control {
	...
	public void save(boolean askFile) {
		...
		if (!extension.equals("." + BAR_EXT)) {
			barFile = barFile + "." + BAR_EXT;
		}
		...
		try {
			...
			try {
				new Station_DAO(Defaults.tempExport).create();
			}
			/*
			 ...
			 */
				...
				new ExceptionPanel(null, dico.entry("SaveProblem"));
			}
			// Zip temp folder
			List<File> fileList = new ArrayList<File>();
			f = new File(Defaults.tempExport);
			try {
				DirectoryUtils.getAllFiles(f, fileList);
			} catch (IOException e) {
				new ExceptionPanel(null, dico.entry("SaveProblem"));
			}
			File zip = new File(barFile);
			try {
				DirectoryUtils.writeZipFile(f, fileList, zip);
			}
			/*
			 * catch (FileNotFoundException e) {}
			 */
	...
	public void open() {
		...
		try {
			...
			try {
				...
				new ExceptionPanel(null, dico.entry("OpenProblem"));
			}
			// populate station
			newStation();
			try {
				new Station_DAO(Defaults.tempExport).read();
			} catch (Exception e) {
				new ExceptionPanel(null, dico.entry("OpenProblem"));
			}
		} finally {
	...
	public void newStation() {
		...
		for (int i = 0; i <= MainFrame.HYDROGRAPH_INDX; i++) {
			refresh(i, "");
		}
	...
	private void refresh(int indx, String name) {
		...
		if (indx == MainFrame.HYDRAULIC_INDX) {
			if (name.equals("")) {
				o = new ConfigHydrauPanel("", false);
			} else {
				o = new ConfigHydrauPanel(name, true);
			}
		} else if (indx == MainFrame.GAUGING_INDX) {
			if (name.equals("")) {
				o = new GaugingPanel("", false);
			} else {
				o = new GaugingPanel(name, true);
			}
		} else if (indx == MainFrame.LIMNI_INDX) {
			if (name.equals("")) {
				o = new LimniPanel("", false);
			} else {
				o = new LimniPanel(name, true);
			}
		} else if (indx == MainFrame.REMNANT_INDX) {
			if (name.equals("")) {
				o = new RemnantErrorPanel();
			} else {
				o = new RemnantErrorPanel();
			}
		} else if (indx == MainFrame.RC_INDX) {
			if (name.equals("")) {
				o = new RatingCurvePanel("", false);
			} else {
				o = new RatingCurvePanel(name, true);
			}
		} else if (indx == MainFrame.HYDROGRAPH_INDX) {
			if (name.equals("")) {
				o = new HydrographPanel("", false);
			} else {
				o = new HydrographPanel(name, true);
			}
		}
	...
	public void renameHydrauConfig(String name) {
		...
		if (newname.equals(Constants.S_EMPTY)) {
			return;
		}
	...
	public void fillHydrauConfigPanel(String id, ConfigHydrauPanel panel) {
		...
		int nc = 0;
		if (hydrau.getControls() == null) {
			nc = 0;
		} else {
			nc = hydrau.getControls().size();
		}
		...
		if (rco != null) {
			...
			if (rco.gethMin() != null) {
				panel.getHmin().setText(Double.toString(rco.gethMin()));
			}
			if (rco.gethMax() != null) {
				panel.getHmax().setText(Double.toString(rco.gethMax()));
			}
			if (rco.gethStep() != null) {
				panel.getHstep().setText(Double.toString(rco.gethStep()));
			}
	...
	public void setCheckboxEnabled(GridBag_CheckBox[][] matrix) {
		...
		for (int i = 0; i < nc; i++) {
			...
			for (int j = i; j < nc; j++) {
				column.add(matrix[i][j].isSelected());
			}
			...
			for (int j = i; j < nc; j++) {
				...
				if (matrix[i][j].isSelected()) {
					if (matrix[i][j].isEnabled()) {
						matrix[i][j].setBackground(Defaults.activeEnabledColor);
					} else {
						matrix[i][j].setBackground(Defaults.activeDisabledColor);
					}
				} else {
					if (matrix[i][j].isEnabled()) {
						matrix[i][j].setBackground(Defaults.passiveEnabledColor);
					} else {
						matrix[i][j].setBackground(Defaults.passiveDisabledColor);
					}
				}
				matrix[i][j].revalidate();
				// revert current cell
	...
	private void writeParInPanel(Distribution prior, GridBag_TextField_Titled val, GridBag_TextField_Titled pom) {
		...
		if (prior.getParval()[0] != null) {
			val.setText(Double.toString(prior.getParval()[0]));
		}
		if (prior.getParval()[1] != null) {
			pom.setText(Double.toString(2 * prior.getParval()[1]));
		}
	}
	...
	public void updateHydrauConfig(ConfigHydrauPanel panel) {
		...
		if (nc > 0) {
			...
			if (conts == null) {
				...
				for (int i = 0; i < nc; i++) {
					conts.add(new HydrauControl());
				}
				hydrau.setControls(conts);
			}
			if (conts.size() > nc) {
				for (int i = nc; i < conts.size(); i++) {
					conts.remove(i);
				}
				hydrau.setControls(conts);
			}
	...
	private Parameter readParInPanel(String name, GridBag_TextField_Titled val, GridBag_TextField_Titled pom) {
		...
		Distribution prior = new Distribution("Gaussian", 2, new String[] { "mean", "standard_deviation" },
				new Double[] { 0.d, 1.d });
		Parameter par = new Parameter();
		m0 = val.getText();
		if (m0.equals(Constants.S_EMPTY)) {
			m = null;
		} else {
			m = safeParse_d(m0);
		}
		sd0 = pom.getText();
		if (sd0.equals(Constants.S_EMPTY)) {
			sd = null;
		} else {
			sd = 0.5 * safeParse_d(sd0);
		}
	...
	public ChartPanel plotHydrau(ConfigHydrauPanel panel) {
		...
		if (hydrau.getPriorEnv().getNx() > 0) {
			chart = hydrau.plot(dico.entry("PriorRC") + " - " + hydrau.getName(), dico.entry("Hunit"),
					dico.entry("Qunit"), ylog);
		}
		return chart;
	}
	...
	public void duplicateHydrauConfig(String original) {
		...
		if (copy.equals(Constants.S_EMPTY)) {
			return;
		}
		// check that newname isn't used
	...
	public boolean setPriorRatingCurveOptions(ConfigHydrauPanel panel) {
		...
		ok = (nsim != Constants.I_UNFEAS);
		if (!ok) {
			return ok;
		}
		hmin = safeParse_D(panel.getHmin().getText());
		if (hmin != null) {
			ok = (hmin != Constants.D_UNFEAS);
			if (!ok) {
				return ok;
			}
		}
		...
		if (hstep != null) {
			ok = (hstep != Constants.D_UNFEAS);
			if (!ok) {
				return ok;
			}
		}
		nstep = safeParse_i(panel.getNstep().getText());
		ok = (nstep != Constants.I_UNFEAS);
		if (!ok) {
			return ok;
		}
	...
	public ChartPanel plotGauging(GaugingPanel panel) {
		...
		ChartPanel chart = null;
		if (!gauging.getGaugings().isEmpty()) {
			chart = gauging.plot(dico.entry("Gaugings") + " - " + gauging.getName(), dico.entry("Hunit"),
					dico.entry("Qunit"), ylog);
		}
		return chart;
	}
	...
	private void fillLimniTable(Limnigraph limni, LimniPanel panel) {
		...
		for (int i = 0; i < limni.getObservations().size(); i++) {
			...
			TM.addRow(new Object[] { obs.getObsDate().toString(), obs.getValue(), limni.getuH()[i],
					limni.getbHindx()[i], limni.getbH()[i] });
		}
		panel.getTable().setModel(TM);
		TM.fireTableDataChanged();
	}
	...
	public ChartPanel plotLimni(LimniPanel panel) {
		...
		ChartPanel chart = null;
		if (!limni.getObservations().isEmpty()) {
			chart = limni.plot(dico.entry("Limnigraph") + " - " + limni.getName(), dico.entry("TimeInYears"),
					dico.entry("Hunit"), ylog);
		}
		return chart;
	}
	...
	public void fillRatingCurvePanel(String id, RatingCurvePanel panel) {
		...
		if (rc.getHydrau_id() != null) {
			panel.getCombo_hydrau().setSelectedIndex(station.getHydrauConfigIndex(rc.getHydrau_id()));
		}
		if (rc.getGauging_id() != null) {
			panel.getCombo_gauging().setSelectedIndex(station.getGaugingIndex(rc.getGauging_id()));
		}
		if (rc.getError_id() != null) {
			panel.getCombo_error().setSelectedIndex(station.getRemnantIndex(rc.getError_id()));
		}
		// Graph
	...
	public ChartPanel plotRatingCurve(RatingCurvePanel panel) {
		...
		if (rc.getEnv_total().getNx() > 0) {
			chart = rc.plot(dico.entry("PostRC") + " - " + rc.getName(), dico.entry("Hunit"), dico.entry("Qunit"),
					ylog);
		}
		return chart;
	}
	...
	public void exportRatingCurve(String name) {
		...
		fc.setAcceptAllFileFilterUsed(false);
		for (int i = 0; i < filter_exportRC.length; i++) {
			fc.addChoosableFileFilter(filter_exportRC[i]);
		}
		String file, file2;
		int returnVal = fc.showOpenDialog(null);
		if (returnVal != JFileChooser.APPROVE_OPTION) {
		...
		if (fext.equalsIgnoreCase(ext)) {
			...
			file2 = file + "." + ext.toLowerCase();
		}
		try {
			if (ext.equalsIgnoreCase("csv")) {
				rc.export_csv(file2);
			} else if (ext.equalsIgnoreCase("dat")) {
				if (rc.getEnv_param().getX().length > RatingCurve.BAREME_NMAX) {
					new InformationPanel(null, dico.entry("BaremeNmaxInfo"));
				}
	...
	public void exportRatingCurveEquation(String name) {
		...
		if (!fext.equalsIgnoreCase("eq")) {
			file = file + ".eq";
		}
		// Retrieve RC and export its equation
		RatingCurve rc = station.getRatingCurve(name);
		try {
			rc.export_equation(file);
		} catch (Exception e) {
			new ExceptionPanel(null, dico.entry("SaveProblem"));
		}
	};
	...
	public void fillHydrographPanel(String id, HydrographPanel panel) {
		...
		if (hyd.getLimni_id() != null) {
			panel.getCombo_limni().setSelectedIndex(station.getLimnigraphIndex(hyd.getLimni_id()));
		}
		if (hyd.getRc_id() != null) {
			panel.getCombo_rc().setSelectedIndex(station.getRatingCurveIndex(hyd.getRc_id()));
		}
		// Graph
	...
	public ChartPanel plotHydrograph(HydrographPanel panel) {
		...
		boolean ylog = panel.getButt_ylog().isSelected();
		if (hyd.getObservations() == null) {
			return null;
		}
		ChartPanel chart = null;
		if (hyd.getObservations().size() > 0) {
			chart = hyd.plot(dico.entry("Hydrograph") + " - " + hyd.getName(), dico.entry("TimeInYears"),
					dico.entry("Qunit"), ylog);
		}
		return chart;
	}
	...
	public void exportHydrograph(String name) {
		...
		try {
			ReadWrite.write(w,
					new String[] { "Year", "Month", "Day", "Hour", "Min", "Sec", "Qmaxpost[m3/s]", "Qlow_stage[m3/s]",
							"Qhigh_stage[m3/s]",
							"Qlow_stage+param[m3/s]", "Qhigh_stage+param[m3/s]", "Qlow_total[m3/s]",
							"Qhigh_total[m3/s]" },
