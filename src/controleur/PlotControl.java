...
public class PlotControl {
	...
	public JPanel priorVsPost_par(RatingCurvePanel panel) {
		...
		int[] sizeRow = new int[ncontrol];
		Arrays.fill(sizeRow, 0);
		int[] sizeCol = new int[4];
		Arrays.fill(sizeCol, 0);
		double[] weightRow = new double[ncontrol];
		Arrays.fill(weightRow, 1.0);
		double[] weightCol = new double[4];
		Arrays.fill(weightCol, 1.0);
	...
	public JPanel priorVsPost_parTable(RatingCurvePanel panel) {
		...
		int ncontrol = h.getControls().size();
		int[] sizeRow = new int[ncontrol + 1];
		Arrays.fill(sizeRow, 0);
		int[] sizeCol = new int[9];
		Arrays.fill(sizeCol, 0);
		double[] weightRow = new double[ncontrol + 1];
		Arrays.fill(weightRow, 0.0);
		double[] weightCol = new double[9];
		Arrays.fill(weightCol, 1.0);
		GridBag_Layout.SetGrid(panout, sizeRow, sizeCol, weightRow, weightCol);
		// headers
		new GridBag_Label(panout, dico.entry("kpar"), config.getFontBigLbl(), Defaults.txtColor, SwingConstants.CENTER,
				1, 0, 2, 1, true, true);
		new GridBag_Label(panout, dico.entry("apar"), config.getFontBigLbl(), Defaults.txtColor, SwingConstants.CENTER,
				3, 0, 2, 1, true, true);
		new GridBag_Label(panout, dico.entry("cpar"), config.getFontBigLbl(), Defaults.txtColor, SwingConstants.CENTER,
				5, 0, 2, 1, true, true);
		new GridBag_Label(panout, dico.entry("bpar"), config.getFontBigLbl(), Defaults.txtColor, SwingConstants.CENTER,
				7, 0, 2, 1, true, true);
		// loop on all controls
		int ai, ci, ki, bi;
		Distribution gauss;
		...
		for (int i = 0; i < ncontrol; i++) {
			...
			x = 0;
			// rower
			new GridBag_Label(panout, dico.entry("Control") + " " + (i + 1), config.getFontBigLbl(), Defaults.txtColor,
					SwingConstants.CENTER, x, i + 1, 1, 1, true, true);
			// K
			gauss = h.getControls().get(i).getK().getPrior();
			// TODO: replace +/- 2*sdev by a proper 95% interval - need to modify BaRatin
			// for that
			priortxt = fmt.format(gauss.getParval()[0]) + plusMinusString + fmt.format(2 * gauss.getParval()[1]);
			posttxt = fmt.format(mcmc[ki][15]) + plusMinusString + fmt.format(2 * mcmc[ki][10]);
			x = x + 1;
			new GridBag_Text_Titled(panout, priortxt, dico.entry("Prior"), config.getFontTxt(), config.getFontLbl(),
					Color.BLUE, Color.BLUE, x, i + 1, 1, 1);
			x = x + 1;
			new GridBag_Text_Titled(panout, posttxt, dico.entry("Posterior"), config.getFontTxt(), config.getFontLbl(),
					Color.RED, Color.RED, x, i + 1, 1, 1);
			// A
			gauss = h.getControls().get(i).getA().getPrior();
			priortxt = fmt.format(gauss.getParval()[0]) + plusMinusString + fmt.format(2 * gauss.getParval()[1]);
			posttxt = fmt.format(mcmc[ai][15]) + plusMinusString + fmt.format(2 * mcmc[ai][10]);
			x = x + 1;
			new GridBag_Text_Titled(panout, priortxt, dico.entry("Prior"), config.getFontTxt(), config.getFontLbl(),
					Color.BLUE, Color.BLUE, x, i + 1, 1, 1);
			x = x + 1;
			...
			gauss = h.getControls().get(i).getC().getPrior();
			priortxt = fmt.format(gauss.getParval()[0]) + plusMinusString + fmt.format(2 * gauss.getParval()[1]);
			posttxt = fmt.format(mcmc[ci][15]) + plusMinusString + fmt.format(2 * mcmc[ci][10]);
			x = x + 1;
			new GridBag_Text_Titled(panout, priortxt, dico.entry("Prior"), config.getFontTxt(), config.getFontLbl(),
					Color.BLUE, Color.BLUE, x, i + 1, 1, 1);
			x = x + 1;
			...
			// B
			priortxt = "    ------    ";
			posttxt = fmt.format(mcmc[bi][15]) + plusMinusString + fmt.format(2 * mcmc[bi][10]);
			x = x + 1;
			new GridBag_Text_Titled(panout, priortxt, dico.entry("Prior"), config.getFontTxt(), config.getFontLbl(),
					Color.BLUE, Color.BLUE, x, i + 1, 1, 1);
			x = x + 1;
	...
	public JPanel priorVsPost_RC(RatingCurvePanel panel) {
		...
		int[] sizeRow = new int[1];
		Arrays.fill(sizeRow, 0);
		int[] sizeCol = new int[1];
		Arrays.fill(sizeCol, 0);
		double[] weightRow = new double[1];
		Arrays.fill(weightRow, 1.0);
		double[] weightCol = new double[1];
		Arrays.fill(weightCol, 1.0);
		GridBag_Layout.SetGrid(panout, sizeRow, sizeCol, weightRow, weightCol);
		Envelop post = rc.getEnv_param();
		Envelop prior = h.getPriorEnv();
		ChartPanel chart_post = post.plot("", "", "]",
				Defaults.plot_postColor, Defaults.plot_postColor_light, 0.6f, Defaults.plot_bkgColor,
				Defaults.plot_gridColor, false);
		ChartPanel chart_prior = prior.plot("", "", "",
				Defaults.plot_priorColor, Defaults.plot_priorColor_light, 0.6f, Defaults.plot_bkgColor,
				Defaults.plot_gridColor, false);
