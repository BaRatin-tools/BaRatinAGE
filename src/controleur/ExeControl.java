...
public class ExeControl {
	...
	private void engine(boolean[] options,GaugingSet gaugings,ConfigHydrau hydrau,
		...
		// prior RC
		if(options[0]==true){ 
			Envelop env=resultController.readEnvelop(new File(Defaults.tempWorkspace,Defaults.results_PriorRC_env+"_010.txt").getAbsolutePath());
			hydrau.setPriorEnv(env);
			Spaghetti spag=resultController.readSpaghetti(new File(Defaults.tempWorkspace,Defaults.results_PriorRC_spag+"_010.txt").getAbsolutePath());
			hydrau.setPriorSpag(spag);
		}
		// raw MCMC
		if(options[1]==true){
			Double[][] y=ReadWrite.read(new File(Defaults.tempWorkspace,mcmc.getFilename()).getAbsolutePath(), Defaults.resultSep, 1);
			rc.setMcmc(y);
		}
		// Post-processing and posterior RC
		if(options[2]==true){
			// Cooked & summary MCMC
			Double[][] y=ReadWrite.read(new File(Defaults.tempWorkspace,Defaults.results_CookedMCMC).getAbsolutePath(),Defaults.resultSep,1);
			rc.setMcmc_cooked(y);
			y=ReadWrite.read(new File(Defaults.tempWorkspace,Defaults.results_SummaryMCMC).getAbsolutePath(),Defaults.resultSep,1,1);
			rc.setMcmc_summary(y);
			// summary HQ
			y=ReadWrite.read(new File(Defaults.tempWorkspace,Defaults.results_SummaryGaugings).getAbsolutePath(),Defaults.resultSep,1);
			rc.setHQ(y);
			// Envelops and Spaghettis
			Envelop env=resultController.readEnvelop(new File(Defaults.tempWorkspace,Defaults.results_PostRC_env+"_011.txt").getAbsolutePath());
			rc.setEnv_total(env);
			env=resultController.readEnvelop(new File(Defaults.tempWorkspace,Defaults.results_PostRC_env+"_010.txt").getAbsolutePath());
			rc.setEnv_param(env);
			Spaghetti spag=resultController.readSpaghetti(new File(Defaults.tempWorkspace,Defaults.results_PostRC_spag+"_011.txt").getAbsolutePath());
			rc.setSpag_total(spag);
			spag=resultController.readSpaghetti(new File(Defaults.tempWorkspace,Defaults.results_PostRC_spag+"_010.txt").getAbsolutePath());
			rc.setSpag_param(spag);
		}
		// h2Q propagation
		if(options[3]==true){
			Envelop env=resultController.readEnvelop(new File(Defaults.tempWorkspace,Defaults.results_Qt_env+"_111.txt").getAbsolutePath());
			env.setX(toYear(limni));
			hydro.setEnv_total(env);
			env=resultController.readEnvelop(new File(Defaults.tempWorkspace,Defaults.results_Qt_env+"_100.txt").getAbsolutePath());
			env.setX(toYear(limni));
			hydro.setEnv_h(env);
			env=resultController.readEnvelop(new File(Defaults.tempWorkspace,Defaults.results_Qt_env+"_110.txt").getAbsolutePath());
			env.setX(toYear(limni));
			hydro.setEnv_hparam(env);
			hydro.setObservations(new ArrayList<Observation>());
			for(int i=0;i<limni.getObservations().size();i++){
				hydro.addObservation(new Observation(env.getMaxpost()[i],limni.getObservations().get(i).getObsDate(),null));
			}
			//TODO: read spag, but this could take several gigas in memory, need to think about it
			if(config.isSaveHydroSpag()){
				Spaghetti spag=resultController.readSpaghetti(new File(Defaults.tempWorkspace,Defaults.results_Qt_spag+"_111.txt").getAbsolutePath());
				spag.setX(toYear(limni));
				hydro.setSpag_total(spag);
				spag=resultController.readSpaghetti(new File(Defaults.tempWorkspace,Defaults.results_Qt_spag+"_110.txt").getAbsolutePath());
				spag.setX(toYear(limni));
				hydro.setSpag_hparam(spag);
				spag=resultController.readSpaghetti(new File(Defaults.tempWorkspace,Defaults.results_Qt_spag+"_100.txt").getAbsolutePath());
				spag.setX(toYear(limni));
		...
		else{new AllDonePanel(null,dico.entry("AllDone"));}
	}

	public void run(boolean[] options,String id_gaugings,String id_hydrau,Integer indx_remnant,String id_mcmc,String id_rc){
		...
		try {
			...
			// update panels
			if(id_hydrau!=null){new ConfigHydrauPanel(id_hydrau,true);}
			if(id_rc!=null){new RatingCurvePanel(id_rc,true);}
		}
		catch (FileNotFoundException e3) {new ExceptionPanel(MainFrame.getInstance(), dico.entry("FileNotFound"));} 
		catch (IOException e3) {new ExceptionPanel(MainFrame.getInstance(), dico.entry("ConfigWriteProblem"));}
		catch (InterruptedException e3) {new ExceptionPanel(MainFrame.getInstance(), dico.entry("RunProblem"));}
		catch (Exception e3) {new ExceptionPanel(MainFrame.getInstance(), dico.entry("UnidentifiedProblem"));}
		finally {MainFrame.getInstance().setCursor(Cursor.getDefaultCursor());}
	}
	...
	public void propagate(String id_hydro){
		...
		try {
			...
			for(int i=0;i<ncol;i++){head[i]="dummy";}
			ReadWrite.write(rc.getMcmc(), head, new File(Defaults.tempWorkspace,mcmc.getFilename()).getAbsolutePath(), Defaults.resultSep);
			engine(new boolean[] {false,false,false,true},gaugings,hydrau,remnant,mcmc,rc,limni,hydro);
			// update panels
			if(id_hydro!=null){new HydrographPanel(id_hydro,true);}
		}
		catch (FileNotFoundException e3) {new ExceptionPanel(MainFrame.getInstance(), dico.entry("FileNotFound"));} 
		catch (IOException e3) {new ExceptionPanel(MainFrame.getInstance(), dico.entry("ConfigWriteProblem"));}
		catch (InterruptedException e3) {new ExceptionPanel(MainFrame.getInstance(), dico.entry("RunProblem"));}
