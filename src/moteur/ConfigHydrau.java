...
public class ConfigHydrau extends Item {
	// classe regroupant une BonnifaitMatrix et les HydrauControl associ�s
	...
	public ChartPanel plot(String title, String xlab, String ylab, boolean ylog) {
		ChartPanel chart = this.getPriorEnv().plot(title, xlab, ylab,
				Defaults.plot_lineColor, Defaults.plot_priorColor_light, 1.0f, Defaults.plot_bkgColor,
		...
		for (int i = 0; i < ncontrol; i++) {
			...
			chart_k[i] = env.plot(title, xlab, ylab,
					Defaults.plot_kPriorColor, Defaults.plot_kPriorColor_light, 0.5f, Defaults.plot_bkgColor,
					Defaults.plot_gridColor, ylog);
			// chart_k[i] = env.plot(title, xlab, ylab,
			// Color.ORANGE, Color.YELLOW, 0.5f, Defaults.plot_bkgColor,
			// Defaults.plot_gridColor, ylog);
			...
			// line_k[i] = Plots.LinePlot(new Double[] { k[0], k[0] }, new Double[] { 0.0,
			// maxi }, title, xlab, ylab,
			// Color.PINK, Color.BLUE, Defaults.plot_gridColor, ylog);
		}
		...
		if (ylog) {
			j.getXYPlot().setRangeAxis(logaxis);
		}
