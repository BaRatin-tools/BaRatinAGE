...
public class Plots {
	...
	public static JFreeChart LinePlot(Double[] x, Double[] y, String title, String xlab, String ylab, Color lineColor,
			Color bkgColor, Color gridColor, boolean ylog) {
		// create XYDataset
		int n = x.length;
		XYSeries s = new XYSeries("data");
		for (int i = 0; i < n; i++) {
			if (ylog & y[i] <= 0) {
				s.add(x[i], Constants.D_TINY);
			} else {
				s.add(x[i], y[i]);
			}
		}
		...
		dataset.addSeries(s);
		// do chart
		JFreeChart chart = ChartFactory.createXYLineChart(title, xlab, ylab, dataset, PlotOrientation.VERTICAL, false,
				true, false);
		// apply options
	...
	public static JFreeChart TimeSeriesPlot(TimeSerie z, String title, String xlab, String ylab,
		...
		for (int i = 0; i < n; i++) {
			...
			if (ylog & obs.getValue() <= 0) {
				y[i] = Constants.D_TINY;
			} else {
				y[i] = obs.getValue();
			}
		}
	...
	public static JFreeChart BandPlot(Double[] x, Double[] y, Double[] ylow, Double[] yhigh, String title, String xlab,
			String ylab,
		...
		for (int i = 0; i < n; i++) {
			if (ylog & yhigh[i] <= 0) {
				series.add(x[i], Constants.D_TINY, Constants.D_TINY, Constants.D_TINY);
			} else if (ylog & y[i] <= 0) {
				series.add(x[i], Constants.D_TINY, Constants.D_TINY, yhigh[i]);
			} else if (ylog & ylow[i] <= 0) {
				series.add(x[i], y[i], Constants.D_TINY, yhigh[i]);
			} else {
		...
		// renderer.setSeriesStroke(0, new BasicStroke(10.0f, BasicStroke.CAP_BUTT,
		// BasicStroke.JOIN_ROUND));
	...
	public static JFreeChart gaussHistPlot(Double mean, Double sd, Double[] sim, String title, String xlab, String ylab,
		...
		pdf.addSeries(s);
		JFreeChart chart_pdf = ChartFactory.createXYLineChart(title, xlab, ylab, pdf, PlotOrientation.VERTICAL, false,
				true, false);
		...
		JFreeChart chart_histo = ChartFactory.createHistogram(title, xlab, ylab, histo, PlotOrientation.VERTICAL, true,
				true, false);
