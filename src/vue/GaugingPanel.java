...
/**
 * Panel for handling gauging sets
 * @author Ben Renard, Irstea Lyon
 */
@SuppressWarnings("serial")
public class GaugingPanel extends ItemPanel implements ActionListener {
	...
	// constants
	private static final FileNameExtensionFilter[] filter_importGaugings={
		new FileNameExtensionFilter("csv",new String[] {"CSV", "csv"}),
		new FileNameExtensionFilter("BAD",new String[] {"BAD", "bad"})};

	public GaugingPanel(String gID,boolean enabled) {
		...
		if(!gID.equals("")){
			...
			controller.fillGaugingPanel(gID,this);
		}
	}
	...
	private void drawInfoPanel(boolean enabled){
		...
		TM = new Custom_TableModel(new String[] {dico.entry("H"),dico.entry("uH"),dico.entry("Q"),dico.entry("uQ"),dico.entry("Active")},
	...
	private void updateGauging(){
		controller.updateGaugingSet(this);
		controller.fillGaugingPanel(this.id.getText().trim(), this);
	}
	...
	public void actionPerformed(ActionEvent e) {
		if(e.getSource().equals(butt_apply)){updateGauging();}		
		else if(e.getSource().equals(butt_browse)){importGauging();}		
		else if(e.getSource().equals(butt_kickout)){controller.kickPlot_gauging(this);}		
		else if(e.getSource().equals(butt_ylog)){controller.fillGaugingPanel(this.id.getText().trim(), this);}		
		else if(e.getSource().equals(butt_legend)){controller.showLegend(this);}
	}

	/**
	 * Handling of the number of decimal places in Q column of the table
	 * @author Ben Renard, modified from http://learn-java-by-example.com/java/control-decimal-places-displayed-jtable-column/
	 */
	static class DecimalFormatRenderer extends DefaultTableCellRenderer {
		...
		public Component getTableCellRendererComponent(JTable table, Object value, 
			...
			Object out; // formatted version of value
			// First format the cell value
			Double Q = (Double)value;				
			if(Q<0.01){out=six.format((Number)value);}
			else if(Q<1.0) {out=three.format((Number)value);}
			else if(Q<10.0) {out=two.format((Number)value);}
			else{out=one.format((Number)value);}
			// And pass it on to parent class
			this.setHorizontalAlignment(RIGHT);
			return super.getTableCellRendererComponent(table,out,isSelected,hasFocus,row,column);
		}
	}
	/////////////////////////////////////////////////////////
	// GETTERS & SETTERS
	...
	/**
	 * @param butt_legend the butt_legend to set
	 */
