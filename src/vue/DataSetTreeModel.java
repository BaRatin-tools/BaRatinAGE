...
public class DataSetTreeModel extends DefaultTreeModel {
	...
	private DefaultMutableTreeNode treeRootNode = new DefaultMutableTreeNode(Dico.getInstance(null).entry("Catalogues"));
	private DefaultMutableTreeNode configTreeNode = new DefaultMutableTreeNode(Dico.getInstance(null).entry("HydrauConf"));
	private DefaultMutableTreeNode gaugingTreeNode = new DefaultMutableTreeNode(Dico.getInstance(null).entry("Gaugings"));
	private DefaultMutableTreeNode errorTreeNode = new DefaultMutableTreeNode(Dico.getInstance(null).entry("RemnantError"));
	private DefaultMutableTreeNode limniTreeNode = new DefaultMutableTreeNode(Dico.getInstance(null).entry("Limnigraph"));
	...
	public DataSetTreeModel() {
		...
		// At this point "this" contains : (1) the five nodes above, completely independent; (2) the default tree model just created, with a dummy root that will never be used 
		// Now add thematic nodes to the "custom" root
		this.treeRootNode.add(configTreeNode);
		this.treeRootNode.add(gaugingTreeNode);
		
		if (Config.getInstance().isExpertMode()){   //if the application is currently running in expert mode, this node is required to display the errors
			this.treeRootNode.add(errorTreeNode);
		}
		
		this.treeRootNode.add(limniTreeNode);
		// And finally, set the root of the tree to our custom root, that now contains all thematic nodes!
		this.setRoot(treeRootNode);          //the setting of the real root of the tree after adding the first level of children
	}
	...
	public void addConfigNode(String name){
		...
		this.reload();  //force la réactualisation, impliquant une mise à jour du rendu du JTree
	}
